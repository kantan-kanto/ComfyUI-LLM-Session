...
- **ComfyUI ネイティブなノード**：チャットやモデル同士の対話を、ComfyUI の workflow 内で実行します
- **セッションファーストの永続化**：会話を永続的な session ID を中心に管理し、対話ログと状態を保存して実行をまたいで再開できるようにします
- **標準版と Simple 版のノード**：標準ノードは主要な実行時設定を UI に出し、Simple ノードは UI を小さく保ちながら高度な調整を JSON 設定に分離します
- **Chat と Dialogue の分離**：ユーザーとモデルの会話、モデル同士の対話を、それぞれ専用のノード種別として扱います
